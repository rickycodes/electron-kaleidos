...
module.exports = function (config, sa) {
  ...
  image.src = (config && config.image) ? config.image : 'http://i.imgur.com/YaZJZac.jpg'
  ...
  (update = (function () {
    return function () {
      if(sa) {
        ...
        var dx, dy, hx, hy
        dx = data[data.length/2] * Math.sin(sa.audio.currentTime) / window.innerWidth
        dy = data[data.length/2] * Math.sin(sa.audio.currentTime) / window.innerHeight
        hx = dx - 0.5
        hy = dy - 0.5
        tx = hx * kaleidoscope.radius * -2
        ty = hy * kaleidoscope.radius * 2
